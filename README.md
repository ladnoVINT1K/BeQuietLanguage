...
## Содержание
- [Введение](#введение)
  - [Что такое BeQuite?](#что-такое-bequite)
...
- [Функции в BeQuite](#функции-в-bequite)
...
### Что такое BeQuite?
BeQuite — это язык программирования, разработанный для образовательных целей и демонстрации принципов работы трансляторов. Язык сочетает простой синтаксис с возможностью изучения внутреннего устройства компиляторов.

### Философия языка
Ключевые принципы BeQuite:
- Простой и понятный синтаксис
- Образовательная направленность
- Полный цикл трансляции от исходного кода до выполнения
...
### Установка
```bash
git clone https://github.com/ladnoVINT1K/BeQuietLanguage
```

### Первый проект
```bequite
// Program.txt
def int main(int arg, massive<massive<char>> args) {
    output('0');
    return 0;
};
```
...
В языке реализованы несколько типов данных:
- `int` - целые числа
- `float` - дробные числа
- `char` - символы, а по совместительству - числовые значения от 0 до 255
- `massive` - массив, непрерывный блок данных заданного типа
- `let` - произвольный тип данных, является аналогом auto из C++
...
int c = a;
float f = c - 0.5;
massive<char> j = {'f', 'o', 'o'};
let h = j;
```

### Работа с массивами
Массивы инициализируются и записываются в фигурных скобках. Для обращения к элементам используются квадратные скобки.

```bequite
massive<int> a = {1, 2, 3, 4, 5};
output(a[0]); // 1
output(a[5]); // index out of range!
```

### Работа со строками
Строки... Не слышали. Строка - массив char.

```bequite
massive<char> str = {'H', 'e', 'l', 'l', 'o', ' ', 'w', 'o', 'r', 'l', 'd', '!'};
...
Ввод-вывод реализован функциями `input` и `output()`. Они не имеют возвращаемого значения.
...
### Объявление функций
Объявление функций происходит по ключевому слову def, после которого следует возвращаемое значение, а затем
следует название функции. В круглых скобках указываем принимаемые параметры. В фигурных скобках пишем реализацию. В конце функции обязателен return!
...
Пример:
```bequite
def void print_string(massive<char> a, int s) {
    for (int i = 0; i < s; i+=1) {
        output(str[i]);
    };
    return;
...
BeQuite поддерживает стандартные условные конструкции if-else.
...
// Также можно написать и без else
int b;
if (b < 6) {
    b += 1;
};
